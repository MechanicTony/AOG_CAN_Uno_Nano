...
//----------------------------------------------------------
//This CAN setup is for AgOpenGPS v5 & FENDT steering controller
//Using Arduino Uno/Nano with CANBUS Shield connected to V-Bus
//Setup for button engage via button on PCB & disengage via V-Bus or Button on PCB
//PWM value drives set curve up & down
//When 'Current Sensor' is selected the work switch will trigger from steer switch (To paint lines)
//----------------------------------------------------------
...
byte data[6] = {0x05, 0x09, 0x0, 0x0A, 0x0, 0x0} ;                  // Fendt Set Curve Data
byte data1[8] = {0x00, 0x00, 0xC0, 0x0C, 0x00, 0x17, 0x02, 0x20} ;  // Fendt Setup 2C Navagation Controller
byte data2[2] = {0x5, 0x19} ;                                       //Fendt Ask For Steering State - Send With Curve Command
...
//--------CAN-Void-Setup-Start------
 ...
 pinMode(CAN0_INT, INPUT);;             // Setting CAN0_INT as input
  ...
  CAN0.init_Filt(0,1,0x0CEF2CF0);       // Init first filter...  
  CAN0.init_Filt(1,1,0x0CEF2CF0);
  
  CAN0.init_Mask(1,1,0xFFFFFF);         // Init second mask... 
  CAN0.init_Filt(2,1,0x0CEF2CF0);       // Init third filter...
  CAN0.init_Filt(3,1,0x0CEF2CF0);       // Init fouth filter...
  CAN0.init_Filt(4,1,0x0CEF2CF0);       // Init fifth filter...
  CAN0.init_Filt(5,1,0x0CEF2CF0);       // Init sixth filter...
  
  CAN0.setMode(MCP_NORMAL);             // Change to normal mode to allow messages to be transmitted

delay (1000);
  CAN0.sendMsgBuf(0x18EEFF2C, 1, 8, data1); //Claim Address, Fendt Only
...
//-------CAN Set Curve START---------------
    ...
    data[0] = 5;
    data[1] = 9;
    if (intendToSteer == 1)data[2] = 3;
    if (intendToSteer == 0)data[2] = 2;
    data[3] = 10;
    if (intendToSteer == 1){
      data[4] = highByte(setCurve*10);
      data[5] = lowByte(setCurve*10);
    }
    else{
      data[4] = 0;
      data[5] = 0;
...
CAN0.sendMsgBuf(0x0CEFF02C, 1, 6, data);  //Send CAN Curve Command
CAN0.sendMsgBuf(0x0CEFF02C, 1, 2, data2); //Send Ask For Current Valve State Message
//-------CAN Set Curve END---------------      
...
//WAS CAN Message
 if (len == 8 && rxBuf[0] == 5 && rxBuf[1] == 10){
      estCurve = ((rxBuf[4] << 8) + rxBuf[5]);
